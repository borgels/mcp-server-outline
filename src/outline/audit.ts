...
export interface OutlineAuditEvent {
  requestId?: string;
  tool: string;
...
export async function writeAuditEvent(event: OutlineAuditEvent): Promise<void> {
  const auditPath = process.env.OUTLINE_AUDIT_LOG;
  if (!auditPath) {
    return;
  }
